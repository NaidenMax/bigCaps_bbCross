...
- **`ENTRY_COOLDOWN_MINUTES = 0` is the new default.** Was `5`. Disables the per-symbol minimum-spacing gate at lines 814-816 and 901-903 of [bt_backtest.py](bt_backtest.py); the underlying check is unchanged in code (it already short-circuits on `<= 0`). Aligns with the live bot's `entry_cooldown_minutes = 0` shipped in the same sprint (see [`../ARCHITECTURE.md`](../ARCHITECTURE.md) §14, "Stacked parents" bullet). With the cooldown gone, `MAX_ENTRIES_PER_SYMBOL_PER_DAY = 10` is the only daily-rate limiter on same-symbol re-entries. Backtests run before this change had a 5-minute cooldown between any two entries on the same symbol; re-run to refresh trade counts (typically a small uptick on consecutive same-bar / next-bar re-cross days).

- **ADX(14) audit columns + two new `analysis.html` bracket sections.** `_compute_daily` and `_compute_30min` in `bt_backtest.py` now compute Wilder's ADX (period set by `ADX_PERIOD = 14` in `bt_config.py`) on both timeframes. Look-ahead safety: the 30-min ADX gets `.shift(1)` matching the rest of the 30-min indicator contract, and daily ADX is read via the worker's `searchsorted(side="left") - 1` (i.e., the most recent COMPLETED daily bar). Two new columns -- `adx14_30m` and `adx14_daily` -- land on every `signals.csv` row and every `trades.csv` leg row. `bt_report.py` adds two parent-level bracket sections after 1d: **1e. PnL by 30m ADX(14) at Entry** and **1f. PnL by Daily ADX(14) at Entry**, both bucketed `<15 / 15-20 / 20-25 / 25-40 / >=40` (textbook Wilder interpretation). **Audit-only -- no entry filter is applied**, so existing trade selection is byte-identical to the prior backtest. Re-run `bt_run.py` to repopulate the new columns; old CSVs will fall back to the "re-run the backtest to populate it" placeholder.
